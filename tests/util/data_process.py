...
class GetKalmanData:
    ...
    def _add_location(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["location"] = df["Face"].astype(str) + "_" + df["Cell"].astype(str) + "_" + df["Point"].astype(str)
        return df

    def _get_faulty_barcodes(self, df, lim:int=448) -> list:
        tl = df.value_counts([self.ID]) < lim
        ls_too_little_welds = tl[tl == True].index
        tm = df.value_counts([self.ID]) > lim
    ...
    def _select_from_limit(self, df) ->pd.DataFrame:
        tobe_remove = self._get_faulty_barcodes(df,self.limit)
        df_filtered = self._filter_rows_by_values(df, self.ID, tobe_remove)
        return df_filtered
    def _shift_data(self, df) ->pd.DataFrame:
        df['target'] = df[self.target].shift(periods= -self.limit)
        df.dropna(axis=0 , subset=['target'], inplace=True)
        return df
        
    def _drop_duplicate(self, df, keep='last')->pd.DataFrame:
        df = df.drop_duplicates(subset=[self.ID,'location'], keep= keep)
        return df

    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        df_out = self._clean_weld_data(df, self.time_col)
        df_out = self._add_location(df_out)
        df_out = self._drop_duplicate(df_out)
        df_out = self._select_from_limit(df_out)
        df_out = self._shift_data(df_out)
        return df_out
...
class Preprocessing:
    ...
    def __init__(self):
        self.time_col = 'Y/M/D hh:mm:ss'
    def _get_faulty_barcodes(self, df, lim1=448, lim2=600) -> list:
    ...
    def _get_good_bad(self, df: pd.DataFrame):
        df['anomaly'] = df.duplicated(subset=['BarCode','location'], keep='last')
        return df

    def preprocess(self, df: pd.DataFrame) -> pd.DataFrame:
        df_out = self.apply_filter(df)
        df_out = self._clean_weld_data(df_out, self.time_col)
        df_out = self._add_location(df_out)
